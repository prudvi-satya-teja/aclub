...
const registrationStatus = async (req, res) => {
    try {
        ...
        return res
            .status(200)
            .json({ status: true, msg: "already registered", registration: registered });
    } catch (err) {
        console.error(err);
        return res.status(500).json({ status: false, msg: "server error" });
    }
...
const giveFeedback = async (req, res) => {
    try {
        ...
        if (!req.body.feedback || !req.body.rating) {
            return res
                .status(400)
                .json({ status: false, msg: "please giving feedback and rating" });
        }
        const user = await User.findOne({ rollNo: req.body.rollNo });
        if (!user) {
        ...
        if (!registration) {
            return res
                .status(400)
                .json({ status: false, msg: "you are not registered for the event" });
        }
...
const feedbackStatus = async (req, res) => {
    try {
        ...
        return res
            .status(200)
            .json({ status: true, msg: "please provide feedback", feedback: registration });
    } catch (err) {
        console.error(err);
        return res.status(500).json({ status: false, msg: "server error" });
    }
...
const getEventFeedback = async (req, res) => {
    try {
        ...
        return res
            .status(200)
            .json({ status: true, msg: "Successfully get Event feedback", feedbacks: feedback });
    } catch (err) {
        console.log(err);
        return res.status(500).json({ status: false, msg: "server error" });
    }
