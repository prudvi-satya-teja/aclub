...
const signup = async (req, res) => {
    try {
        ...
        if (await User.findOne({ phoneNo: req.body.phoneNo })) {
            return res
                .status(400)
                .json({ status: false, msg: "user with same phoneNo already exist" });
        }
...
const forgotPassword = async (req, res) => {
    ...
    try {
        ...
        console.log("otp is  : ", otpManager.otpMap.get(req.body.rollNo));

        const sendEmail = await mailSender.sendMail(
            mail,
            "otp for password resetting",
            `This is one time password valid for 10 minutes ${otp}`
        );
