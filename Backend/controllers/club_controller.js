...
const createClub = async(req, res) => {
    try {
        ...
        if(await Club.findOne({name: req.body.name}) || await Club.findOne({clubId: req.body.clubId})){
          return res.status(201).json({ "status": false, "msg" : "club with name or id already exists"});
        }
        ...
        const club = new Club({ name: req.body.name, clubId: req.body.clubId , clubImage: imagePath , about: req.body.description});
        const newClub = await club.save();
        // console.log(newClub);
        return res.status(201).json({ "status": true, "msg" : "club creation successful", "club details": newClub})
    }
    catch(err) {
        console.error(err);
...
const deleteClub = async(req, res) => {
    try {
        ...
        // console.log(club)
        return res.status(201).json({ "status" : true, "msg": "club successfully deleted", "deleted club": club});
    }
    catch(err) {
        console.error(err);
        return res.status(500).json({ "status": false, "msg" : "server error"})
...
const updateClub = async(req, res) => {
    ...
    try{
        ...
        if(await Club.findOne( club)) {
            ...
            return res.status(200).json({"status" : true, "msg": "club updated successfully", "updated Club": updateClub});
        }
        return res.status(400).json({"status": false, "msg": "No club found with the provided details"});
    }
    catch(err) {
        console.error(err);
        return res.status(500).json({ "status": false, "msg" : "server error"});
