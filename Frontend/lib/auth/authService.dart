...
class AuthService {
  final String baseUrl = 'https://auclub.onrender.com';
  ...
  Future<Map<String, dynamic>> signUser(
      String rollNumber, String password) async {
    final res = await http.post(
      Uri.parse('$baseUrl/auth/login'),
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({"rollNo": rollNumber, "password": password}),
    );
    print("response code:${res.statusCode}");
...
//verify Otp
  ...
  Future<Map<String, dynamic>> resetPass(String password, String rollNo) async {
    final res = await http.post(Uri.parse('$baseUrl/auth/set-forgot-password'),
        headers: {'Content-Type': 'application/json'},
...
//post feedback
  Future<Map<String, dynamic>> giveFeedBack(
      String eventName, String feedback, int rating, String rollNo) async {
    // eventName = Uri.encodeComponent(eventName).replaceAll("+", "%20");

    final res =
        await http.post(Uri.parse('$baseUrl/registrations/give-feedback'),
            headers: {'Content-Type': 'application/json'},
            body: jsonEncode({
              "eventName": eventName,
              "feedback": feedback,
...
// to add memeber to a club
  Future<Map<String, dynamic>> addMember(String first, String last, String roll,
      String? role, String clubId, String phoneNo) async {
    final res = await http.post(
...
//update User details
  Future<Map<String, dynamic>> UpdateUser(String rollNo, String role) async {
    final res = await http.post(
      ...
      body: jsonEncode(
          {"rollNo": rollNo, "role": role, "clubId": Shared().clubId}),
    );
...
// update event
  Future<Map<String, dynamic>> updateEvent(
    ...
    String token =
        "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2NzhhMTA2MWJlZGRmZmE4ZGVmMTIxYTQiLCJhZG1pbiI6dHJ1ZSwicm9sbE5vIjoiMjJhOTFhMDU3MCIsImlhdCI6MTc0MTk1Njk2OCwiZXhwIjoxNzUwNTk2OTY4fQ.Msj8rmceN-rO4MnRs8xOJNPv1QhQD7ULSYEuSUy2-G0"; // Replace with actual token logic
